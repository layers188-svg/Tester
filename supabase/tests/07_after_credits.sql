-- House Dark — After Credits opens only after you have spoken.
--
-- The rule the product turns on: a member may not read anyone else's
-- six words on a film until they have published their own. It is
-- enforced in get_after_credits (migration 0017) rather than in a
-- policy, so it needs testing where it actually lives — a policy test
-- would pass against a function that never consulted one.
--
-- The empty result is part of the contract, not an accident: a member
-- who has published always appears in their own result, so zero rows
-- means "you have not written yours", never "the room is empty".
...
begin
  ...
  perform tests.check('AC.2', 'publishing opens the room',
    v_rows > 0, 'still no rows after publishing');
...
-- 4. The gate is per member, not global: the outsider published before
...
begin
  ...
  perform tests.check('AC.7', 'the gate is judged per member',
    v_rows > 0, 'a member who had published saw a shut room');
end $$;
