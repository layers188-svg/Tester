...
Build a server abstraction around the movie metadata provider.

For production recommendations:

- metadata keys stay server side
- recommendation editorial output is cached by external film ID
- safe six-word copy is versioned
- exact word count is validated
- editorial override is possible
- the same film output is stable across users until a new version is approved
