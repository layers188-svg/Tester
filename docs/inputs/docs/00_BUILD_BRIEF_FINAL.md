...
Use the supplied 20 intent-led themes in `data/house_dark_theme_map.json`.

Examples include:

- `I want to feel tense`
- `I want romance without clichés`
- `I want ambition and obsession`
- `I want friendship and connection`
- `I want something beautiful and strange`

The interface may shorten these labels where needed, but the underlying matching should resolve to the curated House Dark theme map.
...
### Recommendation universe: the supplied House Dark 500

The supplied `data/house_dark_500_films.xlsx` and normalized `data/house_dark_500_films.json` are now the canonical recommendation universe for Trust Us.

This **replaces the earlier idea of an endless or open-ended recommendation repository**.

The corpus contains 500 unique films across 20 intent-led themes, with 25 films per theme. Each film has an approved six-word blind synopsis and search tags.

Trust Us must select from this finite corpus only.

Do not:

- query an LLM for arbitrary new film recommendations
- use an external metadata provider as an unlimited recommendation pool
- expose the 500 titles as a member-facing catalogue
- return multiple titles for comparison
- silently fall back to an infinite external catalogue when a theme is exhausted

External movie data may still be used for availability, verified watch destinations, Library exact-title add and admin enrichment.

The member flow remains:

`theme / intent → ONE film → title + six words → TRUST US / SEEN IT`

`SEEN IT` advances through the same theme without returning to a list and without immediately repeating previously excluded films.

Full implementation contract: `docs/09_FILM_LIBRARY_500.md`.

### Recommendation records

Use the approved record from the 500-film corpus rather than generating a fresh six-word line on every request.
...
The supplied six-word line must remain exactly six words under the project's word-count rules. Future editorial changes must be versioned, validated and human-approved.
...
```ts
progress = clamp(
  (window.scrollY - roomSceneTop) /
  (roomSceneHeight - window.innerHeight),
  0,
  1
)
```
...
## 8A. Preferred visual prototype

Use `prototype_reference/house_dark_v15_preferred_visual_reference.html` as the preferred prototype direction.

The user preferred V15 over the later V16 control experiment. V15's stronger spatial ambition, HD-spine idea, material contrast and award-site influence are the visual starting point.

Do not copy V15 literally into production. Apply the brand system and accessibility rules in this brief, but preserve its level of visual ambition.

The goal is still one coherent House, not five unrelated art directions.
