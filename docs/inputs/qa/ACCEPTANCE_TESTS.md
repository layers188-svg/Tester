...
1. Import validation reports 500 unique active House Dark records.
2. The theme map contains 20 themes with 25 films each.
3. Every supplied blind synopsis validates to exactly six words.
4. Initial member state shows themes, no film catalogue.
5. Select an ambition/obsession theme.
6. Exactly one recommendation appears.
7. The returned film's `hdId` exists in the supplied House Dark 500 corpus.
8. Recommendation contains title and exact six-word line only, plus permitted quiet metadata.
9. No poster, cast, synopsis, rating or reviews.
10. `SEEN IT` replaces the film without returning to a list.
11. New recommendation stays within the same theme while eligible candidates remain.
12. The prior recommendation is in the exclusion set and does not immediately recur.
13. `TRUST US` closes to `THAT'S ALL YOU GET. GO IN BLIND.`
14. Changing theme starts a new recommendation sequence.
15. No member-facing endpoint returns a multi-title catalogue from the 500-film corpus.
16. Stub/disable external movie recommendation calls and confirm Trust Us still functions from the 500-film data.
17. Exhaust a test theme fixture and confirm the defined finite-corpus fallback, not an unlimited external search.
...
## I2. Existing six sample videos

1. Repository inventory identifies the six supplied House Dark sample videos.
2. Each active sample has a manifest entry.
3. Any title-bearing public filename is replaced by an opaque served filename.
4. Launch-used samples pass mobile playback.
5. Launch-used samples have spoiler, rights and human approval state recorded.
6. Missing files are reported by exact path/count rather than silently regenerated.
